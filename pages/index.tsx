...
export default function Dashboard() {
  ...
  // Fixed columns with proper TypeScript types
  const columns = [
    {
      key: 'rank',
      label: '#',
      render: (_value: any, _row: any, index?: number) => (
        <div className="flex items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm ${
            (index ?? 0) === 0 ? 'bg-yellow-500' : (index ?? 0) === 1 ? 'bg-gray-400' : (index ?? 0) === 2 ? 'bg-amber-600' : 'bg-blue-500'
          }`}>
            {(index ?? 0) + 1}
          </div>
        </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            Darts Analytics Dashboard
          </h1>
          <p className="text-xl text-gray-600">
            Comprehensive tournament statistics and performance insights
          </p>
        </div>

        {/* Hero Stats Grid */}
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-8 mb-8">
          <StatCard
            title="Players"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-r from-red-400 to-red-600 p-6 rounded-xl text-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-100 text-sm font-medium">Most 180s</p>
                <p className="text-3xl font-bold">{totalStats.highest180s}</p>
                <p className="text-red-100 text-sm">In one tournament</p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-500 to-purple-600">
            <h3 className="text-xl font-semibold text-white flex items-center">
              ...
              🏆 Top 10 Performers - Hall of Fame
            </h3>
            <p className="text-blue-100 mt-1">Elite players ranked by 3-dart average performance</p>
          </div>
